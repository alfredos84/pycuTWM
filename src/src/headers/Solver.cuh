...
__global__ void kernelSetDiffractionPropagator ( complex_t *eiQz_p, complex_t *eiQz_s, complex_t *eiQz_i, 
						...
						real_t kp, real_t ks, real_t ki, uint32_t pass )
...
__global__ void dAdz( complex_t *dAp, complex_t *dAs,  complex_t *dAi, complex_t *Ap, complex_t *As, complex_t *Ai, 
	real_t kappa_p, real_t kappa_s, real_t kappa_i, real_t dk, real_t z, real_t ap, real_t as, real_t ai, uint32_t pass )
{
	complex_t Im; Im.x = 0; Im.y = 1;
	...
	for (uint32_t idt = 0; idt < NT; idt++){
		if( idx < NX and idy < NY ){
			...
			#ifdef SHG
			dAp[IDX(idx,idy,idt)]  = 1.0f*Im * kappa_p * As[IDX(idx,idy,idt)] * CpxConj(Ap[IDX(idx,idy,idt)]) * CpxExp(+dir*dk*z) - 0.5f*ap*Ap[IDX(idx,idy,idt)] ;
			dAs[IDX(idx,idy,idt)]  = 0.5f*Im * kappa_s * Ap[IDX(idx,idy,idt)] * Ap[IDX(idx,idy,idt)] * CpxExp(-dir*dk*z) - 0.5f*as*As[IDX(idx,idy,idt)] ;
			dAi[IDX(idx,idy,idt)]  = 0.5f*Im * kappa_i * Ap[IDX(idx,idy,idt)] * Ap[IDX(idx,idy,idt)] * CpxExp(-dir*dk*z) - 0.5f*ai*Ai[IDX(idx,idy,idt)] ;
			#endif			
...
{	
	...
	real_t ap = Cr->alpha_crp, as = Cr->alpha_crs, ai = Cr->alpha_cri;	

	//k1 = dAdz(kappas,dk,z,A)
	dAdz<<<grid2D,block2D>>>( k1p_ptr, k1s_ptr, k1i_ptr, Ap_ptr, As_ptr, Ai_ptr, kp, ks, ki, dk, z, ap, as, ai, pass );
	// CHECK(cudaDeviceSynchronize()); 
	...
	// CHECK(cudaDeviceSynchronize());   
	dAdz<<<grid2D,block2D>>>( k2p_ptr, k2s_ptr, k2i_ptr, auxp_ptr, auxs_ptr, auxi_ptr, kp, ks, ki, dk, z+dz/4.0f, ap, as, ai, pass );
	// CHECK(cudaDeviceSynchronize());
	...
	// CHECK(cudaDeviceSynchronize());   
	dAdz<<<grid2D,block2D>>>( k3p_ptr, k3s_ptr, k3i_ptr, auxp_ptr, auxs_ptr, auxi_ptr, kp, ks, ki, dk, z+dz/4.0f, ap, as, ai, pass );
	// CHECK(cudaDeviceSynchronize());
	...
	// CHECK(cudaDeviceSynchronize());   
	dAdz<<<grid2D,block2D>>>( k4p_ptr, k4s_ptr, k4i_ptr, auxp_ptr, auxs_ptr, auxi_ptr, kp, ks, ki, dk, z+dz/2.0f, ap, as, ai, pass );
	// CHECK(cudaDeviceSynchronize());
...
void Solver::run_multipass()
{
	if (check_courant_stability((Cr->dz) , 0.2f*(A->kp)*(Cr->dx)*(Cr->dy)))
	{
		...
		// Reflection coefficients for pump and signal (for now, we consider perfect reflection for fundamental)
		real_t R_p = 1.0f, R_s = 0.95f; 
		double iStart = seconds();	// Timing code
        A->set_plan_diffraction();
        A->set_plan_dispersion();
		A->Ap = A->Api;
		for (int pass = 1; pass <= this->npasses; pass++){
			...
			ssmf(pass);
			if (pass < npasses){
				A->add_phase_in_air(A->lp, A->ls, this->mirror_distances[pass-1], R_p, R_s);
			}
		}
		A->destroy_cufft_plans();
		double iElaps = seconds() - iStart;	// finish timing
