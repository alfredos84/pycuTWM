				...
				real_t R_p, real_t R_s )
{
    ...
    if (idx < NX && idy < NY) {
		...
		// R_p and R_s are mirror reflectivities applied to the fields if needed
        Ap_ptr[IDX(idx, idy, idz)] = sqrt(R_p)*CpxExp(phase_p) * auxp_ptr[IDX(idx, idy, idz)];
        As_ptr[IDX(idx, idy, idz)] = sqrt(R_s)*CpxExp(phase_s) * auxs_ptr[IDX(idx, idy, idz)];
    }
    return;
}
...
public:
	...
	void add_phase_in_air(real_t lp, real_t ls, real_t distance_cm, real_t R_p, real_t R_s);
...
void EFields::add_phase_in_air(real_t lp, real_t ls, real_t distance_cm, real_t R_p, real_t R_s)
{
	...
	// En la CPU (add_phase_in_air)
	// double raw_p = (4.0 * PI * n_p * L_m) / lambda_p_m;
	// double raw_s = (4.0 * PI * n_s * L_m) / lambda_s_m;

	// fmod en double es mucho más preciso para números grandes
	// real_t phase_p = (real_t)std::fmod(raw_p, 2.0 * M_PI);
	// real_t phase_s = (real_t)std::fmod(raw_s, 2.0 * M_PI);

	
	double n_p_d = (double)ref_index_air(lp);
	double n_s_d = (double)ref_index_air(ls);
	double L_m_d = (double)distance_cm * 1e-2;

	// Calcula TODO en double primero
	double full_phase_p = (4.0 * PI * n_p_d * L_m_d) / (double)(lp * 1e-6);
	double full_phase_s = (4.0 * PI * n_s_d * L_m_d) / (double)(ls * 1e-6);

	// Solo ahora, reduce al rango [0, 2pi] y convierte a float para el kernel
	real_t phase_p = (real_t)std::fmod(full_phase_p, 2.0 * PI);
	real_t phase_s = (real_t)std::fmod(full_phase_s, 2.0 * PI);
	...
	addPhase_kernel<<<grid2D, block2D>>>( Ap_ptr, As_ptr, auxp_ptr, auxs_ptr, phase_p, phase_s, R_p, R_s );
    cudaDeviceSynchronize();
	std::cout << std::setprecision(8) << "        ---> Refractive index (np_F, n_SH) = (" << n_p_d << ", " << n_s_d	 << ")" << std::endl;
	std::cout << "        ---> Phase added to fields." << std::endl;
	std::cout << "        ---> \u03C6_F  = " << phase_p << " rad" << std::endl;
	std::cout << "        ---> \u03C6_SH = " << phase_s << " rad" << std::endl;
	std::cout << "        ---> \u0394\u03C6 = \u03C6_F - \u03C6_SH = " << (phase_s - 2*phase_p) << " rad" << std::endl;
