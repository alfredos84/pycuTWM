...
// This class contains the Sellmeier equations and properties for PPLN (periodically poled lithium niobate) nonlinear crystal.
// Sellmeier equations from Gayer et al., Appl. Phys. B 91, 343–348 (2008).

class PPLN : public Crystal
{
public:
    bool QPM;
    ...
    PPLN(real_t _LX, real_t _LY, real_t _Lcr, real_t _T, real_t _Lambda, real_t _lp, real_t _ls, real_t _li)
    {
        ...
        name = "PPLN";
        QPM = true;
        d33 = 27.0e-6;      dQ = 2.0 * d33 / PI;
        alpha_crp = 0.02e-4; alpha_crs = 0.002e-4; alpha_cri = 0.002e-4;
    ...
    ~PPLN() override { printf("PPLN deleted\n"); }
    ...
    std::string get_crystal_type() const override { return "PPLN"; }
};

// ---------- Methods  ----------

void PPLN::set_dk(real_t dk)
{
    this->dk = dk;
    return ;
}

/** Returns the PPLN extraordinary refractive index */
real_t PPLN::n(real_t L, real_t T)
{
    // Values from Gayer et al.
