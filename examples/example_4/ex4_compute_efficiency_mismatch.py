...
ax.plot(x*Lcr, y/np.max(y), marker='o', markersize=3, linewidth=1, label='SHG efficiency')

ax.set_xlabel(r"$\Delta k$L$_{cr}$") #L$_{cr}$
ax.set_ylabel(r"Normalized SHG efficiency (a.u.)")
# ax.set_title(r"Norm. SHG efficiency for MgO:PPLN crystal")
ax.grid(True, which='both', linestyle='--', alpha=0.6)
# ax.legend(loc='best')
plt.tight_layout()
plt.show()
fig.savefig("efficiency_dk.png", dpi=150, bbox_inches='tight')
plt.close(fig)
np.savetxt('efficiency_vs_dk.dat', np.column_stack((x, y)), fmt='%.6f')
