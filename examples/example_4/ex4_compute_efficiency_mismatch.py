...
This example calculates the normalized SHG efficiency
as a function of the mismatch factor, Δk
...
x, y = dk, 100*output/in_power
ax.plot(x, y, marker='o', linewidth=2, label='SHG efficiency')

ax.set_xlabel(r"$\Delta k$") #L$_{cr}$
ax.set_ylabel(r"Normalized SHG efficiency (a.u.)")
ax.set_title(r"Normalized SHG efficiency for MgO:PPLN crystal")
ax.grid(True, which='both', linestyle='--', alpha=0.6)
ax.legend(loc='best')
plt.tight_layout()
plt.show()
plt.show()
fig.savefig("efficiency_dk_waist_"+str(waist_um)+".png", dpi=150, bbox_inches='tight')
plt.close(fig)
...
print("Max efficiency at dk = "+ str(x_max/Lcr))