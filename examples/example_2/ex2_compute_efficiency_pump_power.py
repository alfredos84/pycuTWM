...
if run_code:
    run_cutwm_sweep(config_path=json_path,
                    ...
                    values=pump_powers,
                    exe_path="../../src/./cuTWM",
                    out_dir_base=f"results/pump_power",
                    keep_config=True,
                    source_dir=".",             
                    value_decimals=3
    )
...
for pump in pump_powers:
    ...
    folder = f"results/pump_power/simulation_pump_power_W_{pump:.3f}"
    print(folder)
...
ax.set_title(f"SHG efficiency for " + crystal_type + "crystal")
ax.grid(True, which='both', linestyle='--', alpha=0.6)
# ax.legend(loc='best')
ax.set_ylim(0, 50)
plt.tight_layout()
# plt.show()
fig.savefig(f"Power_scaling.png", dpi=150, bbox_inches='tight')
plt.close(fig)
